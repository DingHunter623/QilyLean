...
["const version='20260903-ddz-fast-knowledge-v155-v158-v159-v160-v161-v162-v163-v164-v169'",'data-qily-ddz-core="v158"','window.__PURE_DDZ_STYLE_READY__=Promise.resolve();','data-qily-ddz-fast-shell="v155"','data-qily-ddz-virtual-landscape="v154"',"const chain=['js/ddz-core-v155.js'];",'id="welcome-start"','id="welcome-landscape"','id="v120-landscape-toggle"','/site-dock-share-runtime-v1.js?','<p id="hint-message" class="hint-message" aria-hidden="true"></p>','无牌可压时自动喊“不要”并轮到下家'].forEach(x=>assert(ddz.includes(x),'斗地主公开入口契约缺失：'+x));
...
const dock=read('site-dock-share-runtime-v1.js');assert(dock.includes('Floating Dock Authoritative Runtime V5.8'),'Dock V5.8 缺失');assert(dock.includes('__qilyFloatingDockUnifiedV58'),'Dock V5.8 防重复装载标记缺失');assert(dock.includes("ORDER=['home','top','back','previous','search','current','contact']"),'七按钮顺序漂移');assert(dock.includes('setOwnedLabel'),'Dock 单一文字所有权缺失');assert(dock.includes('overflow-x:visible!important')&&dock.includes('scroll-snap-type:none!important')&&dock.includes('mobile-fixed-bottom-compact-navigation'),'移动端 Dock 固定底栏/七栏紧凑布局契约缺失');assert(dock.includes('function openContactPage()'),'联系我们完整页面路由缺失');assert(dock.includes("w.open(url,'_blank','noopener,noreferrer')"),'联系我们未明确新窗口/新标签打开');assert(dock.includes("if(action==='contact'){openContactPage();return;}"),'Dock 联系我们仍未绑定完整联系页');assert(dock.includes('function isExcluded(){return false;}'),'普通公开页面不得再排除斗地主 Dock');assert(!dock.includes("mask.classList.add('show')"),'Dock 联系我们不得再打开二维码模态框');assert(!/new\s+MutationObserver\s*\(/.test(dock),'Dock 禁止 MutationObserver 持续重建');
...
console.log('PASS: V36 behavior owners remain stable: official sticky Header, native-range navigation, Google Translate V1.4, canonical Dock V5.8 and the isolated DDZ V155/V164 adaptive mobile-landscape integration.');
