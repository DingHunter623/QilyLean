...
assert(semanticsJs.includes('Interaction Semantics Runtime V1.5')&&semanticsJs.includes('__qilyInteractionSemanticsV15'),'交互语义 V1.5 缺失');
for(const token of ['.brief-action-strip>span','.tag-row>li','qily-primary-nav-scroll-rail','qily-primary-nav-scroll-thumb',"w.addEventListener('pointermove',move,{passive:false})",'nav.scrollLeft=startScroll-dx'])assert(semanticsJs.includes(token),'交互语义运行时缺失：'+token);
assert(semanticsCss.includes('Interaction Semantics V1.4'),'交互语义 CSS 视觉基线缺失');
...
console.log('PASS: behavior owners remain stable on V28 baseline, including iPhone-safe navigation dragging and the exact three-language public picker.');
