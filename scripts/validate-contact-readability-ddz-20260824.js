...
const dock=read('site-dock-share-runtime-v1.js');assert(dock.includes('Floating Dock Authoritative Runtime V5.7'),'Dock V5.7 缺失');assert(dock.includes('__qilyFloatingDockUnifiedV57'),'Dock V5.7 防重复装载标记缺失');assert(dock.includes("ORDER=['home','top','back','previous','search','current','contact']"),'七按钮顺序漂移');assert(dock.includes('setOwnedLabel'),'Dock 单一文字所有权缺失');assert(dock.includes('overflow-x:hidden!important')&&dock.includes('mobile-fixed-bottom-navigation'),'移动端 Dock 固定底栏/禁止横向滚动契约缺失');assert(dock.includes('function openContactPage()'),'联系我们完整页面路由缺失');assert(dock.includes("w.open(url,'_blank','noopener,noreferrer')"),'联系我们未明确新窗口/新标签打开');assert(dock.includes("if(action==='contact'){openContactPage();return;}"),'Dock 联系我们仍未绑定完整联系页');assert(dock.includes('function isExcluded(){return false;}'),'普通公开页面不得再排除斗地主 Dock');assert(!dock.includes("mask.classList.add('show')"),'Dock 联系我们不得再打开二维码模态框');assert(!/new\s+MutationObserver\s*\(/.test(dock),'Dock 禁止 MutationObserver 持续重建');
...
console.log('PASS: V36 behavior owners remain stable: official sticky Header, native-range navigation, Google Translate V1.4, canonical Dock V5.7 and the isolated DDZ V155/V164 adaptive mobile-landscape integration.');
