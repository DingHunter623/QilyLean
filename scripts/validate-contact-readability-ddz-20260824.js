...
["const version='20260828-elder-ux-v127'","loadStyle('css/card-comfort-v122.css')",'id="welcome-start"'].forEach(x=>assert(ddz.includes(x),'斗地主公开入口契约缺失：'+x));

const closure=read('tools/pure-ddz/game/css/r8-closure-v128.css');
assert(closure.includes('Pure DDZ R10 Closure V130'),'斗地主 R10 闭环缺失');
assert(closure.includes('Pure DDZ R9 Closure V129'),'斗地主 R9 兼容标记缺失');
assert(closure.includes('left:50%!important')&&closure.includes('transform:translateX(-50%)!important'),'本人牌区未锁定桌面中轴');
...
assert(dock.includes('Floating Dock Authoritative Runtime V5.3'),'Dock V5.3 缺失');
...
assert(contact.includes('Contact Route V13.3'),'Contact Route V13.3 缺失');
assert(contact.includes('__qilyFloatingDockUnifiedV53'),'Contact recovery 未切换 Dock V5.3');
assert(contact.includes('20260829-authority-v53'),'Contact recovery 仍可能回写旧 Dock');

const semanticsJs=read('site-interaction-semantics-v1.js'),semanticsCss=read('site-interaction-semantics-v1.css');
assert(semanticsJs.includes('Interaction Semantics Runtime V1.3')&&semanticsJs.includes('__qilyInteractionSemanticsV13'),'交互语义 V1.3 缺失');
...
assert(semanticsCss.includes('Interaction Semantics V1.3'),'交互语义 CSS V1.3 缺失');
