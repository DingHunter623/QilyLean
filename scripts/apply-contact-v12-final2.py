...
skip_names={'apply-contact-v12-final.js','apply-contact-v12-final2.py','unify-wechat-official-url-contact-v12.js','preflight-contact-v12-cooperation.js'}
...
t=re.sub(r'<p class=\\"qily-wechat\\">.*?</p><button class=\\"qily-copy-wechat\\" type=\\"button\\">.*?</button>',newrow,t,count=1,flags=re.S)
...
assert r'data-qily-wechat-copy=\"Qily259\"' in t
assert 'qily-copy-wechat' not in t
p.write_text(t,'utf-8')

# Shared visual layer; one deleted row funds larger typography.
...
# Project cooperation: entire row is the button frame.
...
assert n==1 or 'wechat-contact-action' in t
t=re.sub(r"\s*wechatButton\.addEventListener\('click',function\(\)\{.*?\}\);",'',t,count=1,flags=re.S)
assert 'wechat-contact-action' in t
p.write_text(t,'utf-8')

# OPL: source HTML is embedded in a JS string, therefore escaped quotes are authoritative.
p=root/'knowledge/terminology.html';t=p.read_text('utf-8')
old_attr=r'data-opl-copy-wechat=\"Qily259\"'
new_attr=r'data-opl-copy-wechat=\"Qily259\" data-qily-wechat-copy=\"Qily259\"'
if old_attr in t and r'data-qily-wechat-copy=\"Qily259\"' not in t:t=t.replace(old_attr,new_attr,1)
repl="""  if(wechatCopy){
...
assert r'微信：<a href=\"#copy-wechat\"' in t
assert r'data-qily-wechat-copy=\"Qily259\"' in t
p.write_text(t,'utf-8')

# Legacy contact panel follows copy-first/ask-before-open.
...
# Exact product QA. Required confirmation is the sole permitted legacy noun.
for f in [root/'site-navigation-core.js',root/'cooperation/index.html',root/'knowledge/terminology.html']:
    ...
    assert '官网网址' not in c, f'{f.name} still has 官网网址'
    assert '微信号' not in c, f'{f.name} still has 微信号'
assert 'QILY-CONTACT-INTERACTION-V12:START' in (root/'site-shell.css').read_text('utf-8')
print('FINAL2 V12 PASS; generic sources changed:',changed)
