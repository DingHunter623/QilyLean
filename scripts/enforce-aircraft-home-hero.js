...
const hero=[START,'<section class="qily-aircraft-brand-hero" aria-label="QilyLean｜启力精益飞机品牌主视觉">','  <figure>','    <picture>',`      <source type="image/webp" srcset="${WEBP}">`,`      <img src="${PNG}" alt="${ALT}" width="${sourceWidth}" height="${sourceHeight}" loading="eager" decoding="async" fetchpriority="high">`,'    </picture>','  </figure>','</section>',END].join('\n');
...
const heroHead=`<link id="qilyAircraftHeroStylesheetV1" rel="stylesheet" href="${CSS}">\n<link id="qilyAircraftHeroPreloadV1" rel="preload" as="image" href="${WEBP}" type="image/webp" fetchpriority="high">\n`;
const baselineAnchor=/<script\b[^>]*data-qily-translation-safety-bootstrap=/i;
html=baselineAnchor.test(html)?html.replace(baselineAnchor,heroHead+'$&'):html.replace('</head>',heroHead+'</head>');
...
console.log(`QilyLean homepage aircraft hero materialized from canonical 官网首图.png (${sourceWidth}x${sourceHeight}) with a Q98 WebP transport derivative and untouched PNG fallback.`);
