...
must(css,'Interaction Semantics V1.4','Semantics V1.4');must(css,'.qily-primary-nav-scroll-rail','Persistent nav rail');must(css,'.qily-primary-nav-scroll-thumb','Nav rail thumb');must(css,'.brief-action-strip>span','Static brief labels');must(css,'.tag-row>li','Static tag rows');forbid(css,'content:"回\\A顶部"','Duplicate top label');forbid(css,'content:"回\\A上一层"','Duplicate back label');must(css,'.overview-card>.tag','Waste number contrast');
must(js,'__qilyInteractionSemanticsV14','Semantics JS V14');must(js,'qily-primary-nav-scroll-rail','Nav rail runtime');must(js,'.brief-action-strip>span','Static token runtime');must(js,'PROJECT_EVIDENCE','Evidence map');must(js,'addTrustLinks','Trust links');
...
must(mat,"const BASELINE_VERSION='20260829-sitewide-visual-closure-v27'",'V27');must(mat,'20260829-dock-functional-public-v134','Contact V134 owner');must(mat,'20260829-authority-v54','Dock V54 owner');must(mat,'20260829-r11-semantics-v14','Semantics V13 owner');must(mat,'20260829-r12-v132','DDZ R12 owner');
...
for(const file of files()){
 ...
 if(!html.includes('/site-interaction-semantics-v1.css?v=20260829-r11-semantics-v14')||!html.includes('/site-interaction-semantics-v1.js?v=20260829-r11-semantics-v14'))fail.push(`${file}: semantics stale`);
