...
must(route,'Contact Route V13.4','Contact V13.4');must(route,'__qilyFloatingDockUnifiedV54','Contact Dock V54');must(contactMat,'20260829-dock-functional-public-v134','Contact materializer V134');must(contactMat,'20260829-authority-v54','Contact materializer V54');
...
must(mat,"const BASELINE_VERSION='20260831-native-picker-grade-readability-v29'",'V29');must(mat,'20260829-dock-functional-public-v134','Contact V134 owner');must(mat,'20260829-authority-v54','Dock V54 owner');must(mat,'20260830-r11-semantics-v15-ios-drag','Semantics V15 owner');must(mat,'20260830-r11-semantics-v14-visual-v3-vi-teal','VI rail owner');must(mat,'20260831-native-picker-stability-v9','Stable picker owner');must(mat,'20260831-project-grade-readability-v3','Project grade owner');must(mat,'20260829-r12-v132','DDZ R12 owner');
...
for(const file of files()){
 ...
 const pickerCurrent=html.includes('/site-translation-public-ui-v1.js?v=20260831-native-picker-stability-v9');
 const pickerPrevious=html.includes('/site-translation-public-ui-v1.js?v=20260830-public-language-picker-v8-stable');
 if(!pickerCurrent&&!pickerPrevious)fail.push(`${file}: picker stale`);
...
if(pages<460||navPages<460||contactPages<470)fail.push(`coverage pages=${pages} nav=${navPages} contact=${contactPages}`);if(fail.length)throw new Error(`V29 sitewide remediation failed:\n${fail.slice(0,40).join('\n')}`);
console.log(`PASS: V29 sitewide remediation covers ${pages} pages; ${owners} ownership artifacts remain shell-free; persistent nav rail, native-stable picker queue, readable project grades, Dock V5.4/Contact V13.4 and DDZ V132 are current.`);
