...
const globalKnowledge=f=>/^global-knowledge\//i.test(f);
...
must(dock,'Floating Dock Authoritative Runtime V5.8','Dock V5.8');must(dock,'__qilyFloatingDockUnifiedV58','Dock V58 guard');must(dock,'setOwnedLabel','Dock label owner');must(dock,'mobile-fixed-bottom-footer-navigation','Mobile Dock fixed-bottom compact layout');must(dock,'overflow-x:visible!important','Mobile Dock horizontal scrolling disabled');must(dock,'function openContactPage()','Full contact route');must(dock,"w.open(url,'_blank','noopener,noreferrer')",'New-tab contact route');must(dock,'function isExcluded(){return false;}','Normal public pages share canonical Dock');forbid(dock,"mask.classList.add('show')",'Dock contact modal');if(/new\s+MutationObserver\s*\(/.test(dock))throw new Error('Dock MutationObserver forbidden');
must(route,'Contact Route V13.4','Contact V13.4');must(route,'__qilyFloatingDockUnifiedV54','Contact Dock backward-compatibility marker');must(contactMat,'20260829-dock-functional-public-v134','Contact materializer V134');must(contactMat,'20260906-authority-v58-mobile-swipe-fixed-bottom','Contact authoritative Dock entry');must(contactMat,'global shell ownership untouched','Contact ownership boundary');forbid(contactMat,'const UI=','Contact shell rewrite');
...
must(cn,'/assets/qilylean-vi-v2.css?v=20260924-cn-vi-v25-header-shell-parity','CN production shared VI');
must(cn,'/assets/cn-nav-rail-v1.js?v=20260925-nav-rail-v9-resource-collab','CN production navigation runtime');
...
let pages=0,navPages=0,contactPages=0,owners=0,cnPages=0,gkPages=0,fail=[];
for(const file of files()){
  const html=read(file);if(ownership(file)){owners++;continue;}if(chinaSite(file)){cnPages++;continue;}if(globalKnowledge(file)){gkPages++;continue;}if(!/<\/head>/i.test(html))continue;pages++;
  if(file==='tools/pure-ddz/index.html'){
    ...
    if(!/\/site-dock-share-runtime-v1\.js\?v=20260906-authority-v58-mobile-swipe-fixed-bottom/.test(html))fail.push(`${file}: DDZ Dock V5.8 stale`);
    if(html.includes("loadStyle('css/ddz-core-v155.css')")||html.includes('/site-navigation.js?'))fail.push(`${file}: retired DDZ waterfall returned`);
    continue;
  }
  ...
  if(/\/site-dock-share-runtime-v1\.js/.test(html)&&!/\/site-dock-share-runtime-v1\.js\?v=(?:20260829-authority-v54|20260906-authority-v58-mobile-swipe-fixed-bottom)/.test(html))fail.push(`${file}: Dock stale`);
}
...
console.log(`PASS: V34 production remediation covers ${pages} primary-site pages; ${owners} ownership artifacts, ${cnPages} China-site files and ${gkPages} Global Knowledge pages are correctly separated; primary international pages use V32, Global Knowledge keeps its isolated VI, China production uses its independent shared VI, and DDZ V155/V164 retains its isolated fast shell.`);
