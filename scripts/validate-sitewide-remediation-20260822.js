...
must(dock,'Floating Dock Authoritative Runtime V5.8','Dock V5.8');must(dock,'__qilyFloatingDockUnifiedV58','Dock V58 guard');must(dock,'setOwnedLabel','Dock label owner');must(dock,'mobile-fixed-bottom-compact-navigation','Mobile Dock fixed-bottom compact layout');must(dock,'overflow-x:visible!important','Mobile Dock horizontal scrolling disabled');must(dock,'function openContactPage()','Full contact route');must(dock,"w.open(url,'_blank','noopener,noreferrer')",'New-tab contact route');must(dock,'function isExcluded(){return false;}','Normal public pages share canonical Dock');forbid(dock,"mask.classList.add('show')",'Dock contact modal');if(/new\s+MutationObserver\s*\(/.test(dock))throw new Error('Dock MutationObserver forbidden');
...
must(cn,'/assets/qilylean-vi-v2.css?v=20260922-cn-vi-v20-current-module','CN production shared VI');
