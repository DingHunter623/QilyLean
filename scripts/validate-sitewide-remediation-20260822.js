...
/* Sitewide remediation compatibility gate | V30 | 2026-08-31 */
...
must(safe,'Safe In-Page Translation V7','Safe translation V7');must(safe,"runtime:'safe-inpage-v7'",'Safe translation runtime');must(safe,'data-qily-header-utility','Translation header utility');
...
must(mat,"const BASELINE_VERSION='20260831-safe-translation-nav-range-v30'",'V30');must(mat,'20260829-dock-functional-public-v134','Contact V134 owner');must(mat,'20260829-authority-v54','Dock V54 owner');must(mat,'20260831-r11-semantics-v17-native-range','Semantics V17 owner');must(mat,'20260830-r11-semantics-v14-visual-v3-vi-teal','VI rail owner');must(mat,'20260831-safe-inpage-v7-header-utility','Safe translation owner');must(mat,'20260831-unified-components-v29-native-range','Visual components owner');must(mat,'20260831-r7-single-responsibility-v11-safe-translation','Shell V11 owner');must(mat,'20260831-project-grade-readability-v3','Project grade owner');must(mat,'20260829-r12-v132','DDZ R12 owner');

const legacyTranslation=['site-translation-public-ui-v1.js','site-translation-public-ui-v1.css','site-translation-progress-v1.js','site-translation-progress-v1.css','site-global-language-v1.css','site-global-language-v3.js'];
...
for(const file of files()){
  ...
  if(!html.includes('/site-translation-safe-runtime-v1.js?v=20260831-safe-inpage-v7-header-utility')||!html.includes('data-qily-translation-safe-direct="v7"'))fail.push(`${file}: translation stale`);
...
if(fail.length)throw new Error(`V30 sitewide remediation failed:\n${fail.slice(0,40).join('\n')}`);
console.log(`PASS: V30 sitewide remediation covers ${pages} pages; ${owners} ownership artifacts remain shell-free; Safe Translation V7, native range nav V1.7, readable project grades, Dock V5.4/Contact V13.4 and DDZ V132 are current.`);
