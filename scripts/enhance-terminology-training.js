...
`;
  const block=/\/\* terminology-(?:training-v2|compact-v3|search-v4) \*\/[\s\S]*?<\/style>/;
  return block.test(page)?page.replace(block,css+'</style>'):page.replace('</style>',css+'</style>');
}
...
var results=document.createElement('div');results.className='term-search-results';results.setAttribute('aria-live','polite');results.innerHTML='<div class="module-heading"><h2>搜索结果</h2><p>仅匹配术语代码、英文全称、中文名称和专用别名，并按相关度排序；点击词条可返回原分类位置。</p></div><div class="term-grid" id="termSearchGrid"></div>';empty.parentNode.insertBefore(results,empty.nextSibling);var grid=document.getElementById('termSearchGrid');
...
function locate(card){input.value='';render();requestAnimationFrame(function(){card.scrollIntoView({behavior:'smooth',block:'center'});card.classList.remove('term-focus');void card.offsetWidth;card.classList.add('term-focus');setTimeout(function(){card.classList.remove('term-focus');},1900);});}
function clone(original){var hit=original.cloneNode(true);hit.removeAttribute('id');hit.classList.add('term-search-hit');hit.setAttribute('role','button');hit.setAttribute('tabindex','0');var hint=document.createElement('div');hint.className='term-jump-hint';hint.textContent='点击定位原词条';hit.appendChild(hint);hit.addEventListener('click',function(){locate(original);});hit.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();locate(original);}});return hit;}
