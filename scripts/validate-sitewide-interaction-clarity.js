...
const HTML_BUILD = '20260817-atomic-first-paint-v3';
...
function validateRuntimeSource() {
  ...
  assert(cleanRuntime.includes("ATTEMPT='qily_site_refresh_attempt_v1'") && cleanRuntime.includes('if(requested||tried()){fallback();return}') && cleanRuntime.includes('if(!active||active===BUILD)fresh();else refresh()'), 'Marker-preserving bounded stale-document retry contract is missing from the materializer.');
