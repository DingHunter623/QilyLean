...
/* Refresh an already-materialized V158 prefetch block to the current V160 cache key without adding another owner. */
source=source.replace(/20260903-ddz-fast-knowledge-v155-v158(?:-v159)?(?:-v160)?/g,CACHE);
...
for(const token of ['function primePureDdzV158(){','data-qily-ddz-prefetch','/tools/pure-ddz/','ddz-core-v155.css?v='+CACHE,'ddz-core-v155.js?v='+CACHE]){
  if(!source.includes(token))throw new Error(`DDZ V160 entry prefetch contract missing: ${token}`);
}
if(source!==before)fs.writeFileSync(file,source.endsWith('\n')?source:source+'\n');
console.log('Pure DDZ V160 homepage entry materialized: direct route + idle/intent prefetch + current cache + current public product copy.');
