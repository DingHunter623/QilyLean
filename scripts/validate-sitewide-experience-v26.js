...
forbid(ddzHtml,'name="screen-orientation"','DDZ forced orientation');forbid(ddzHtml,'name="x5-orientation"','DDZ forced X5 orientation');forbid(ddzHtml,'qilyPureDdzR8ClosureV128','DDZ retired closure');must(ddzHtml,'Promise.all(chain.map','DDZ parallel resource download');must(ddzHtml,'},1800);','DDZ early interface reveal');must(ddzHtml,'20260903-ddz-mobile-landscape-v153','DDZ V153 cache');must(ddzHtml,"loadStyle('css/mobile-landscape-v153.css')",'DDZ landscape stylesheet');must(ddzHtml,'id="v120-landscape-toggle"','DDZ toolbar landscape entry');must(ddzHtml,'id="welcome-landscape"','DDZ welcome landscape entry');must(ddzHtml,'/site-dock-share-runtime-v1.js?','DDZ canonical Dock runtime');
...
const ddzLandscape=read('tools/pure-ddz/game/css/mobile-landscape-v153.css');must(ddzLandscape,'html.ddz-mobile-landscape body.ddz-site-page','DDZ landscape scoped owner');must(ddzLandscape,'var(--ddz-mobile-vh,390px)','DDZ actual viewport height');must(ddzLandscape,'env(safe-area-inset-right)','DDZ landscape safe area');forbid(ddzLandscape,'#floatDock','DDZ landscape Dock ownership');
const ddzGame=read('tools/pure-ddz/game/js/game.js');must(ddzGame,"const VERSION = '1.5.2'",'DDZ game core');must(ddzGame,"auto?'不要':'您不要'",'DDZ Hint auto-pass narration');
const ddzVisual=read('tools/pure-ddz/game/js/visual-v120.js');must(ddzVisual,"version:'1.2.4-mobile-landscape-adaptive'",'DDZ mobile landscape runtime');must(ddzVisual,'function syncViewportProfile()','DDZ viewport profile');must(ddzVisual,'screen.orientation?.lock','DDZ supported-browser landscape lock');must(ddzVisual,'document.documentElement.requestFullscreen','DDZ user-gesture fullscreen request');must(ddzVisual,'window.PureDDZTest.hint()','DDZ single Hint behavior owner');forbid(ddzVisual,"document.addEventListener('click',requestLandscape",'DDZ page-wide forced landscape click');
...
console.log('PASS: V26 compatibility remains intact on V32: official sticky Header, native-range navigation, Google Translate V1.4, canonical Dock V5.5, DDZ V153 adaptive landscape and unified VI authority.');
