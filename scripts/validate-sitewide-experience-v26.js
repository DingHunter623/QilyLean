...
must(dock,'Floating Dock Authoritative Runtime V5.8','Dock runtime');must(dock,'__qilyFloatingDockUnifiedV58','Dock runtime marker');must(dock,"ORDER=['home','top','back','previous','search','current','contact']",'Dock seven-action order');for(const label of ['首页','顶部','上一层级','上一网页','本站搜索','分享当前','联系我们'])must(dock,label,`Dock action ${label}`);must(dock,'grid-template-columns:repeat(7,minmax(0,1fr))!important','Dock desktop module grid');must(dock,'border-radius:8px!important','Dock rectangular geometry');must(dock,'overflow-x:visible!important','Mobile Dock horizontal scrolling disabled');must(dock,'scroll-snap-type:none!important','Mobile Dock scroll snapping disabled');must(dock,'touch-action:pan-y pinch-zoom!important','Mobile Dock vertical gesture ownership');must(dock,'mobile-fixed-bottom-footer-navigation','Dock mobile fixed-bottom layout');must(dock,'position:fixed!important','Dock fixed-bottom placement');must(dock,'function isExcluded(){return false;}','Canonical Dock public-page coverage');forbid(dock,'border-radius:50%!important','Retired circular Dock geometry');
