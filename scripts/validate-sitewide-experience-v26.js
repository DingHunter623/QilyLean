...
must(dock,'Floating Dock Authoritative Runtime V5.7','Dock runtime');must(dock,'__qilyFloatingDockUnifiedV57','Dock runtime marker');must(dock,"ORDER=['home','top','back','previous','search','current','contact']",'Dock seven-action order');for(const label of ['回首页','回顶部','回上一层级','回上一网页','本站搜索','分享当前页','联系我们'])must(dock,label,`Dock action ${label}`);must(dock,'grid-template-columns:repeat(7,minmax(0,1fr))!important','Dock desktop module grid');must(dock,'border-radius:8px!important','Dock rectangular geometry');must(dock,'overflow-x:hidden!important','Dock mobile horizontal scroll disabled');must(dock,'scroll-snap-type:none!important','Dock mobile snap disabled');must(dock,'mobile-fixed-bottom-navigation','Dock mobile fixed-bottom layout');must(dock,'position:fixed!important','Dock fixed-bottom placement');must(dock,'function isExcluded(){return false;}','Canonical Dock public-page coverage');forbid(dock,'border-radius:50%!important','Retired circular Dock geometry');
