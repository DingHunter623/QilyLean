...
must(dock,'Floating Dock Authoritative Runtime V5.8','Dock runtime');must(dock,'__qilyFloatingDockUnifiedV58','Dock runtime marker');must(dock,"ORDER=['home','top','back','previous','search','current','contact']",'Dock seven-action order');for(const label of ['回首页','回顶部','回上一层级','回上一网页','本站搜索','分享当前页','联系我们'])must(dock,label,`Dock action ${label}`);must(dock,'grid-template-columns:repeat(7,minmax(0,1fr))!important','Dock desktop module grid');must(dock,'border-radius:8px!important','Dock rectangular geometry');must(dock,'overflow-x:auto!important','Dock mobile horizontal swipe');must(dock,'scroll-snap-type:x proximity!important','Dock mobile snap');must(dock,'-webkit-overflow-scrolling:touch!important','Dock mobile inertial swipe');must(dock,'mobile-fixed-bottom-swipe-navigation','Dock mobile fixed-bottom layout');must(dock,'position:fixed!important','Dock fixed-bottom placement');must(dock,'function isExcluded(){return false;}','Canonical Dock public-page coverage');forbid(dock,'border-radius:50%!important','Retired circular Dock geometry');
...
console.log('PASS: V26 compatibility remains intact on V32: official sticky Header, native-range navigation, Google Translate V1.4, rectangular fixed-bottom swipe Dock V5.8, DDZ V155 isolated fast shell and unified VI authority.');
