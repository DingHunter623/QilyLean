...
/* QilyLean core-service runtime scoping v6｜2026-08-20
 * 项目合作增强脚本只允许保留在 cooperation/index.html；
 * 普通页面全部清除，避免无效请求；合作页使用事件驱动的轻量 V10 运行时。
 */
const fs=require('fs');
const path=require('path');
const root=path.resolve(__dirname,'..');
const css='/site-core-service-dock-closure-v1.css?v=20260804-core-service-dock-v3';
const js='/site-core-service-dock-closure-v1.js?v=20260822-cooperation-dock-v10';
const cssTag=`<link id="qilyCoreServiceDockClosureStylesheet" rel="stylesheet" href="${css}">`;
const jsTag=`  <script defer data-qily-core-service-dock-closure="v10" src="${js}"></script>`;
...
function install(html,rel){
  ...
  if(cooperation){
    const firstPaintAsset=/<style\b[^>]*id=["']qilyDockCriticalV6["']/i;
    out=out.replace(/^[ \t]*(?=<style\b[^>]*id=["']qilyDockCriticalV6["'])/mi,'');
    if(firstPaintAsset.test(out))out=out.replace(firstPaintAsset,`${cssTag}\n${jsTag}\n$&`);
    else out=out.replace(/<\/head>/i,`${cssTag}\n${jsTag}\n</head>`);
  }
  return out;
}
...
process.stdout.write(`Core-service runtime scoping v6 checked ${checked} HTML pages; refreshed ${changed}; cooperation pages kept=${kept}; redundant non-cooperation assets removed=${removed}; cooperation runtime=v10.\n`);
