 ...
 * 普通页面全部清除，避免无效请求；合作页使用事件驱动的轻量 V9 运行时。
 */
const fs=require('fs');
const path=require('path');
const root=path.resolve(__dirname,'..');
const css='/site-core-service-dock-closure-v1.css?v=20260804-core-service-dock-v3';
const js='/site-core-service-dock-closure-v1.js?v=20260820-cooperation-dock-v9';
const cssTag=`<link id="qilyCoreServiceDockClosureStylesheet" rel="stylesheet" href="${css}">`;
const jsTag=`  <script defer data-qily-core-service-dock-closure="v9" src="${js}"></script>`;
...
process.stdout.write(`Core-service runtime scoping v6 checked ${checked} HTML pages; refreshed ${changed}; cooperation pages kept=${kept}; redundant non-cooperation assets removed=${removed}; cooperation runtime=v9.\n`);
