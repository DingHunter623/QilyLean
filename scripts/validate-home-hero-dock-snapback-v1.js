...
/* Homepage / Dock snapback compatibility gate | V33 | Dock V5.7 fixed-bottom | 2026-09-06 */
...
must(dock,'Floating Dock Authoritative Runtime V5.7','Dock V5.7');must(dock,'__qilyFloatingDockUnifiedV57','Dock V57 guard');must(dock,"ORDER=['home','top','back','previous','search','current','contact']",'Dock seven-action order');must(dock,'setOwnedLabel','Dock label owner');must(dock,'position:fixed!important','Dock fixed-bottom position');must(dock,'overflow-x:hidden!important','Mobile Dock no horizontal scroll');must(dock,'mobile-fixed-bottom-navigation','Mobile Dock fixed-bottom marker');must(dock,"w.open(url,'_blank','noopener,noreferrer')",'Contact new tab');must(dock,'function isExcluded(){return false;}','Public Dock coverage');forbid(dock,"mask.classList.add('show')",'Dock modal');forbid(dock,'overflow-x:auto!important','Retired swipe rail');
...
const resources=[
  ...
  [['/site-dock-share-runtime-v1.js?v=20260906-authority-v57-mobile-fixed-bottom'],'Dock'],
...
console.log('PASS: V33 homepage guard protects aircraft hero, VI deep-teal native-range navigation, Google Translate V1.4, readable project grades and authoritative fixed-bottom Dock V5.7 with mobile horizontal scrolling disabled.');