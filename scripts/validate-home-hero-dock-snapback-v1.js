...
/* Homepage / Dock snapback compatibility gate | V33 | Dock V5.8 fixed-bottom swipe | 2026-09-06 */
...
must(dock,'Floating Dock Authoritative Runtime V5.8','Dock V5.8');must(dock,'__qilyFloatingDockUnifiedV58','Dock V58 guard');must(dock,"ORDER=['home','top','back','previous','search','current','contact']",'Dock seven-action order');must(dock,'setOwnedLabel','Dock label owner');must(dock,'position:fixed!important','Dock fixed-bottom position');must(dock,'overflow-x:auto!important','Mobile Dock horizontal swipe');must(dock,'mobile-fixed-bottom-swipe-navigation','Mobile Dock fixed-bottom marker');must(dock,"w.open(url,'_blank','noopener,noreferrer')",'Contact new tab');must(dock,'function isExcluded(){return false;}','Public Dock coverage');forbid(dock,"mask.classList.add('show')",'Dock modal');must(dock,'scroll-snap-type:x proximity!important','Mobile Dock snap rail');must(dock,'-webkit-overflow-scrolling:touch!important','Mobile Dock inertial swipe');
...
console.log('PASS: V33 homepage guard protects aircraft hero, VI deep-teal native-range navigation, Google Translate V1.4, readable project grades and authoritative fixed-bottom Dock V5.8 with native mobile horizontal swiping.');