...
const ROUTE='/site-contact-route-v1.js?v=20260829-dock-functional-public-v132';
...
for(const relative of trackedHtml()){
  ...
  let next=source.replace(/\/site-contact-route-v1\.js(?:\?v=[^"']*)?/g,ROUTE).replace(/data-qily-contact-route-direct=["'][^"']*["']/g,'data-qily-contact-route-direct="v13.2"');
...
if(process.argv.includes('--check')){
  ...
  for(const [label,html] of [['contact',contact],['home',home],['pure-ddz',ddz]]){if(!html.includes(ROUTE))throw new Error(`${label} does not use Contact Route V13.2.`);if(!html.includes(REDLINE))throw new Error(`${label} does not use public redline V2.`);if(/site-ui-consistency-v1\.js/.test(html)&&!html.includes(UI))throw new Error(`${label} still loads stale UI consistency runtime.`);}
  ...
  process.stdout.write(`PASS: R9 contact baseline current on ${covered} HTML pages; Contact Route V13.2; direct Dock V5.2 refs ${dockCovered}; UI V7 refs ${uiCovered}.\n`);
}else process.stdout.write(`R9 contact materialized on ${changed} HTML file(s); route coverage ${covered}; Contact V13.2; Dock V5.2 refs ${dockCovered}; UI refs ${uiCovered}.\n`);
