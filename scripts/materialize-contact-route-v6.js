...
const ROUTE='/site-contact-route-v1.js?v=20260829-dock-functional-public-v133';
const DOCK='/site-dock-share-runtime-v1.js?v=20260829-authority-v53';
...
for(const relative of trackedHtml()){
  ...
  let next=source.replace(/\/site-contact-route-v1\.js(?:\?v=[^"']*)?/g,ROUTE).replace(/data-qily-contact-route-direct=["'][^"']*["']/g,'data-qily-contact-route-direct="v13.3"');
...
if(process.argv.includes('--check')){
  if(changed)throw new Error(`R10 contact materialization stale on ${changed} HTML file(s).`);if(covered<470)throw new Error(`Contact route coverage unexpectedly low: ${covered}.`);
  ...
  for(const [label,html] of [['contact',contact],['home',home],['pure-ddz',ddz]]){if(!html.includes(ROUTE))throw new Error(`${label} does not use Contact Route V13.3.`);if(!html.includes(REDLINE))throw new Error(`${label} does not use public redline V2.`);if(/site-ui-consistency-v1\.js/.test(html)&&!html.includes(UI))throw new Error(`${label} still loads stale UI consistency runtime.`);}
  if(/site-dock-share-runtime-v1\.js/.test(ddz)&&!ddz.includes(DOCK))throw new Error('Pure DDZ direct Dock runtime is not cache-busted to authoritative V5.3.');
  process.stdout.write(`PASS: R10 contact baseline current on ${covered} HTML pages; Contact Route V13.3; direct Dock V5.3 refs ${dockCovered}; UI V7 refs ${uiCovered}.\n`);
}else process.stdout.write(`R10 contact materialized on ${changed} HTML file(s); route coverage ${covered}; Contact V13.3; Dock V5.3 refs ${dockCovered}; UI refs ${uiCovered}.\n`);
