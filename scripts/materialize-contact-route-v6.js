 ...
 * - owns Contact Route V13.4 references, direct authoritative Dock V5.8 references and clean contact-map markup;
...
const DOCK='/site-dock-share-runtime-v1.js?v=20260906-authority-v58-mobile-swipe-fixed-bottom';
...
const DDZ_FAST_SOURCE_DOCK='/site-dock-share-runtime-v1.js?v=20260906-authority-v58-mobile-swipe-fixed-bottom';
...
if(checkOnly){
  ...
  if(ddzFastRoute){
    ...
    if(!ddz.includes(DDZ_FAST_SOURCE_DOCK)&&!ddz.includes(DOCK))throw new Error('Pure DDZ fast route does not use an approved Dock V5.8 runtime.');
  }else{
  ...
  process.stdout.write(`PASS: R12.2 contact baseline current on ${covered} HTML pages; Contact Route V13.4; direct Dock V5.8 refs ${dockCovered}; global shell ownership untouched.\n`);
}else process.stdout.write(`R12.2 contact materialized on ${changed} HTML file(s); route coverage ${covered}; Contact V13.4; Dock V5.8 refs ${dockCovered}; global shell ownership untouched.\n`);
