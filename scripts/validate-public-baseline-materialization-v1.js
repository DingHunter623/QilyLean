...
for(const file of files()){
  ...
  if(/\/site-ui-consistency-v1\.js(?:\?v=[^"']*)?/.test(html)){shellPages++;if(!html.includes('/site-ui-consistency-v1.js?v=20260831-r7-single-responsibility-v9-native-picker'))failures.push(`${file}: shared shell stale`);}
...
for(const sample of ['index.html','trust/index.html','experience/index.html','projects/index.html','qilylean/daily/2026-08-25.html','qilylean/daily/2026-07-29.html']){
  const html=read(sample);if(!html.includes('/site-header-axis-v1.css?v=20260829-primary-navigation-safe-scroll-v7'))failures.push(`${sample}: R11 header absent`);if(!html.includes('/site-interaction-semantics-v1.js?v=20260830-r11-semantics-v15-ios-drag'))failures.push(`${sample}: Semantics V1.5 absent`);if(!html.includes('/site-translation-public-ui-v1.js?v=20260831-native-picker-stability-v9'))failures.push(`${sample}: native-stable picker absent`);if(!html.includes('/site-header-project-integrity-v2.css?v=20260831-project-grade-readability-v3'))failures.push(`${sample}: project grade integrity absent`);if(!html.includes('/site-contact-route-v1.js?v=20260829-dock-functional-public-v134'))failures.push(`${sample}: Contact V13.4 absent`);if(!html.includes('/site-public-redline-closure-v2.css?v=20260830-annotated-v2'))failures.push(`${sample}: Redline V2 CSS absent`);if(!html.includes('/site-public-redline-closure-v2.js?v=20260830-annotated-v2'))failures.push(`${sample}: Redline V2 JS absent`);
}
...
if(failures.length)throw new Error(`V29 public materialization failed (${failures.length}):\n${failures.slice(0,50).join('\n')}`);
console.log(`PASS: ${audited} public pages carry V29 native-picker stability + project-grade readability with the existing Public Redline Closure V2 baseline.`);
