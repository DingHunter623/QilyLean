...
const required=[
  ...
  '/site-content-contrast-guard-v1.css?v=20260826-sitewide-content-contrast-v4',
  '/site-content-contrast-guard-v1.js?v=20260826-sitewide-content-contrast-v4'
];
const failures=[];
let audited=0;
for(const relative of trackedHtml()){
  ...
  if(!html.includes('data-qily-content-contrast-direct="v4"'))failures.push(`${relative}: content contrast v4 marker missing`);
...
for(const sample of samples){
  if(!fs.existsSync(path.join(root,sample)))failures.push(`${sample}: required remediation sample missing`);
    ...
    if(!html.includes('/site-content-contrast-guard-v1.js?v=20260826-sitewide-content-contrast-v4'))failures.push(`${sample}: content contrast v4 baseline absent`);
    if(!html.includes('data-qily-content-contrast-direct="v4"'))failures.push(`${sample}: content contrast v4 marker absent`);
...
process.stdout.write(`PASS: ${audited} tracked HTML pages carry one unified safe translation/readability/mobile-navigation baseline with content contrast v4; no retired external-proxy translator remains referenced.\n`);