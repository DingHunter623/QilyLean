...
for(const file of files()){
  const html=read(file);if(!/<\/head>/i.test(html)||ownership(file)||file===DDZ||file.startsWith(GLOBAL_KNOWLEDGE_PREFIX))continue;audited++;
  ...
  if(/\/site-dock-share-runtime-v1\.js(?:\?v=[^"']*)?/.test(html)&&!html.includes(DOCK))failures.push(`${file}: direct Dock stale`);
}
...
const ddz=read(DDZ);
for(const token of ['20260903-ddz-fast-knowledge-v155','data-qily-ddz-fast-shell="v155"','fast-site-shell-v155.js?v=20260915-ddz-free-choice-v176'])if(!ddz.includes(token))failures.push(`${DDZ}: missing fast-route contract ${token}`);
...
console.log(`PASS: ${audited} primary-site public pages use the V32 baseline and Dock V5.8 authority; Global Knowledge remains under its isolated VI, while DDZ V155/V164 remains the explicitly isolated fast route.`);
