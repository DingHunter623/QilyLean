...
function verify(rel, html) {
  ...
  assert(first.includes("ATTEMPT='qily_site_refresh_attempt_v1'")&&first.includes('w.sessionStorage.getItem(ATTEMPT)===BUILD')&&first.includes('if(requested||tried()){fresh();return}'),`${rel}: bounded stale-document retry contract missing`);
