...
if(!page.includes('terminology-sponsor-v1.js')) page=page.replace('</body>', '<script defer src="'+sponsorScript+'"></script>\n</body>');
...
if(fs.existsSync(path.join(root,brief))){let b=read(brief);b=b.replace(/高层Sponsor(?!（)/g,'高层Sponsor（项目发起人／主责高层）');write(brief,b);}

const dataFile='qilylean/site-data.json';
const data=JSON.parse(read(dataFile));
data.generatedAt='2026-08-01';
data.terminology=data.terminology||{};data.terminology.total=181;data.terminology.lessonTotal=181;
data.search=data.search||{};data.search.terminologyTotal=181;
...
try{
  ...
  if(entries){
    ...
    if(!Array.isArray(index)){
      index.meta=index.meta||{};index.meta.generatedAt='2026-08-01';index.meta.terminologyTotal=181;index.meta.totalEntries=entries.length;
    }
    data.search.indexedEntries=entries.length;
    write(indexFile,JSON.stringify(index,null,2));
    write(dataFile,JSON.stringify(data,null,2));
  }
}catch(error){console.warn('Search index update skipped:',error.message);}

function addSitemap(file){
 if(!fs.existsSync(path.join(root,file)))return;
 let xml=read(file);const loc='https://qilylean.com/knowledge/terminology/sponsor.html';
 if(!xml.includes(loc)) xml=xml.replace('</urlset>','  <url><loc>'+loc+'</loc><lastmod>2026-08-01</lastmod></url>\n</urlset>');
 write(file,xml);
}
addSitemap('sitemap.xml');addSitemap('sitemap-core.xml');
console.log('Published Sponsor terminology, OPL, brief annotation, statistics, search and sitemap synchronization.');
