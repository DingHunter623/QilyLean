...
const certificateGuard = read('site-certificate-facts-v2.js');
...
if (!capabilities.includes('/qilylean/chatgpt-lean-certificate-web.jpg')) fail('Historical certificate web image was not restored');
if (!capabilities.includes('/qilylean/chatgpt-lean-certificate.png')) fail('Historical certificate original image link missing');
if (!capabilities.includes('GPT-LE-2025-0422')) fail('Certificate number missing');
if (!capabilities.includes('2025年4月22日')) fail('Certificate date missing');
if (!capabilities.includes('OpenAI · ChatGPT / Codex')) fail('OpenAI related platform/tool field missing');
if (!capabilities.includes('平台官方网站，非本证书核验入口')) fail('OpenAI website boundary missing');
if (capabilities.includes('/assets/certificates/ai-large-model-engineer.jpg')) fail('Broken certificate placeholder path reappeared');
if (!capabilities.includes('/site-certificate-facts-v2.js')) fail('Certificate runtime fact guard missing');
if (!certificateGuard.includes('GPT-LE-2025-0422')) fail('Certificate fact runtime is incomplete');
