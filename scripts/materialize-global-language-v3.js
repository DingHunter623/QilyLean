...
/* QilyLean Global Translation Dual Route V2 materializer｜2026-08-25 */
const fs = require('fs');
...
function materialize(source) {
  ...
  next = next.replace(/\s*<script\b[^>]*(?:data-qily-global-language-direct|data-qily-google-translate-direct|data-qily-web-translate-direct)=["'][^"']+["'][^>]*><\/script>\s*/gi, '\n');
...
if (checkOnly && changed.length) {
  throw new Error(`Global Translation Dual Route V2 materialization stale: ${changed.slice(0, 30).join(', ')}${changed.length > 30 ? ` … +${changed.length - 30}` : ''}`);
}

process.stdout.write(`Global Translation Dual Route V2 ${checkOnly ? 'check passed' : 'materialized'}: ${changed.length} tracked HTML file(s).\n`);
