...
/* QilyLean Sitewide Public Baseline Materializer V15｜2026-08-27
 * Translation recovery + Dock semantic-icon closure:
 ...
 * - failed target-language work restores source mode atomically with no recovery overlay.
...
const BASELINE_VERSION='20260827-translation-source-recovery-v15';
const SAFE_VERSION = '20260827-source-recovery-v4';
const CONSISTENCY = '/site-ui-consistency-v1.js?v=20260827-translation-dock-resource-v46';
...
const PROGRESS_JS='/site-translation-progress-v1.js?v=20260827-source-recovery-v4';
...
function materialize(source){let next=source;next=neutralizeFirstPaint(next);next=next.replace(/\/site-ui-consistency-v1\.js(?:\?v=[^"']*)?/g,CONSISTENCY);next=next.replace(/\/site-navigation\.js(?:\?v=[^"']*)?/g,NAVIGATION);next=next.replace(/\/site-parent-navigation-v3\.js(?:\?v=[^"']*)?/g,PARENT_NAV);next=next.replace(/\/site-dock-share-runtime-v1\.js(?:\?v=[^"']*)?/g,DOCK_SHARE);next=next.replace(/\/site-core-service-dock-closure-v1\.js(?:\?v=[^"']*)?/g,CORE_SERVICE_DOCK);next=next.replace(/\/assets\/contact\/wechat-contact-card\.svg(?:\?v=[^"']*)?/g,WECHAT_CONTACT_ASSET);next=removeScriptByMarker(next);next=removeLegacyManagedScripts(next);next=removeManagedStyles(next);
  const tags=[
    '<script data-qily-translation-safety-bootstrap="inpage-v3">window.__qilyGlobalTranslationDualRouteV2=true;window.__qilyGoogleTranslateOnDemandV1=true;window.__qilyGlobalLanguageV31=true;window.__qilyGlobalLanguageV3=true;window.__qilyGlobalLanguageV2=true;window.__qilyGlobalLanguageV1=true;</script>',
    ...
    `<script defer data-qily-translation-safe-direct="inpage-v3" src="${SAFE_RUNTIME}"></script>`,
    `<script defer data-qily-translation-public-ui-direct="visitor-v2" src="${PUBLIC_UI_JS}"></script>`,
    `<script defer data-qily-translation-progress-direct="bilingual-v3" src="${PROGRESS_JS}"></script>`,
