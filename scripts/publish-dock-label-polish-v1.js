...
/* QilyLean dock first-paint + navigation polish v3｜2026-08-15
 * 目标：
 * 1) 静态HTML首帧即显示“分享 / 官方网址”，彻底消除“分享官网 → 分享官方网址”的跳变；
 * 2) 在旧合作增强脚本可能改写按钮时，以单按钮 MutationObserver 在同一渲染帧内锁回最终文案；
 * 3) 非项目合作页面移除历史 core-service-dock JS/CSS，减少无效请求与运行时DOM工作；
 * 4) 缓存升级 consistency=v6，Fast Native 仍保持V6协议但刷新到性能补丁查询版本。
 */
const fs=require('fs');
const path=require('path');
const root=path.resolve(__dirname,'..');
const CONSISTENCY_SRC='/site-ui-consistency-v1.js?v=20260816-nav-current-v7';
const FAST_NATIVE_SRC='/site-music-persistent-navigation-v1.js?v=20260815-prefetch-v6p1';
const DIRECT_TAG=`<script defer data-qily-ui-consistency="nav-current-v7" src="${CONSISTENCY_SRC}"></script>`;
...
const LOCK_SCRIPT=`<script data-qily-dock-firstpaint-lock="v6">(function(d){'use strict';var s=d.querySelector('#floatDock [data-action="share"]');if(!s)return;var h='<span class="qily-share-label-line qily-share-label-primary">分享</span><span class="qily-share-label-line qily-share-label-url">官方网址</span>';function f(){if(s.innerHTML!==h)s.innerHTML=h;if(s.getAttribute('aria-label')!=='分享官方网址')s.setAttribute('aria-label','分享官方网址');if(s.getAttribute('title')!=='分享官方网址')s.setAttribute('title','分享官方网址')}f();if('MutationObserver'in window){var busy=false;new MutationObserver(function(){if(busy)return;busy=true;f();busy=false}).observe(s,{subtree:true,childList:true,characterData:true,attributes:true,attributeFilter:['aria-label','title']})}})(document);</script>`;
...
function patchHtml(html,rel){
  ...
  out=out.replace(/<\/head>/i,`${CRITICAL_STYLE}\n</head>`);
...
if(!wrapper.includes(CONSISTENCY_SRC))throw new Error('site-navigation.js dock-label cache version v6 missing');
process.stdout.write(`Dock first-paint v3 checked ${checked} public HTML pages; refreshed ${changed}; removed redundant core-service dock assets from ${trimmed} non-cooperation pages; consistency=${CONSISTENCY_SRC}; fast-native=${FAST_NATIVE_SRC}.\n`);
