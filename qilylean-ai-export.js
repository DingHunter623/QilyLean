...
function toPngDataUrl(source,size){
...
function exportAssets(){
  return Promise.all([
    toPngDataUrl(HOME_QR_SRC,360),
    waitForContactQr(0).then(function(source){return toPngDataUrl(source,420);})
...
async function exportWord(){
  ...
  try{
    ...
    var html='<!doctype html><html><head><meta charset="utf-8"><title>QilyLean AI 对话记录</title><style>'+
      ...
      '<div class="cover"><h1>QilyLean AI 对话记录</h1><div class="sub">导出时间：'+esc(time.display)+'</div><div class="principle">简单化 · 专业化 · 标准化</div></div>'+sections+contactBlock(assets)+
