...
grep -Fq '/assets/cn-translate-baidu-v1.css?v=20260918-translate-v3' "$INDEX_FILE" || { echo "ERROR: CN homepage translator stylesheet is missing."; exit 1; }
grep -Fq '/assets/cn-translate-baidu-v1.js?v=20260918-translate-v3' "$INDEX_FILE" || { echo "ERROR: CN homepage translator runtime is missing."; exit 1; }
...
grep -Fq 'qily-cn-nav-shell' "$CN_NAV_RAIL_JS" || { echo "ERROR: CN primary-nav scroll shell is missing."; exit 1; }
grep -Fq -- '--qily-nav-scroll-track:#f3e6cf' "$CN_VI_FILE" || { echo "ERROR: CN nav rail track must match international gold VI."; exit 1; }
grep -Fq -- '--qily-nav-scroll-thumb:#caa15f' "$CN_VI_FILE" || { echo "ERROR: CN nav rail thumb must match international gold VI."; exit 1; }
