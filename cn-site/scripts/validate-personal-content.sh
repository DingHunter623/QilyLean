...
[[ -f "$CN_TRANSLATE_CSS" ]] || { echo "ERROR: CN in-page translation stylesheet is missing."; exit 1; }
...
grep -Fq 'data-public-visual-scope="factory-layout-system-v4"' "$PRACTICE_FACTORY" || { echo "ERROR: CN factory-layout public visual must use the professional system V4 composition."; exit 1; }
...
grep -Fq 'qily-factory-layout-system-v4' "$PRACTICE_FACTORY" || { echo "ERROR: CN factory-layout system summary layer is missing."; exit 1; }
grep -Fq 'viewBox="0 0 1400 788"' "$PRACTICE_FACTORY" || { echo "ERROR: CN factory-layout V4 must retain 16:9-safe composition."; exit 1; }
...
grep -Fq '/assets/qilylean-vi-v2.css?v=20260926-cn-vi-v28-footer-visual-parity' "$INDEX_FILE" || { echo "ERROR: CN homepage unified VI cache version is missing."; exit 1; }
...
grep -Fq 'qily-primary-nav-scroll-thumb' "$CN_NAV_RAIL_JS" || { echo "ERROR: CN custom navigation thumb runtime is missing."; exit 1; }
grep -Fq 'mousedown' "$CN_NAV_RAIL_JS" || { echo "ERROR: CN mouse drag runtime is missing."; exit 1; }
grep -Fq 'touchstart' "$CN_NAV_RAIL_JS" || { echo "ERROR: CN touch drag runtime is missing."; exit 1; }
...
grep -Fq '/assets/practice/factory-layout.svg?v=20260923-practice-map-v4-system' "$PRACTICE_PAGE" || { echo "ERROR: CN factory planning representative practice is missing."; exit 1; }
