...
- 国际外链视觉语义统一：主导航中的 `精选简报 ↗` 与 `资源协同 ↗` 均以 `↗` 标识跨域；中国站首页 `进入 Global Knowledge ↗` 同样使用 `↗`。三者必须使用 `target="_blank"`、`rel="noopener noreferrer"` 与 `data-qily-external="international"`，不得出现一个有箭头、一个无箭头的差异。
