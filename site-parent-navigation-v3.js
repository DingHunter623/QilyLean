/* QilyLean 悬浮栏父级导航轻量版 v4.2｜2026-08-25
 * 仅负责“回上一层”与导航当前态；中文标签只在 zh-CN 权威源模式下校正。
 */
(function(d,w){
  'use strict';
  if(w.__qilyParentNavigationV42)return;
  w.__qilyParentNavigationV42=true;
  w.__qilyParentNavigationV3=true;
  d.documentElement.classList.remove('qily-shell-pending','qily-r2-first-paint-pending');
  function normalizedPath(path){var value=(path||'/').replace(/\/index\.html$/,'/').replace(/\/{2,}/g,'/');return value.length>1?value.replace(/\/+$/,'/'):'/';}
  function sourceMode(){return (d.documentElement.getAttribute('data-qily-language')||'zh-CN')==='zh-CN'}
  function configuredParent(){var body=d.body;var value=(body&&body.getAttribute('data-parent-route'))||'';if(value)return value;var link=d.querySelector('link[rel="up"][href]');return link?link.getAttribute('href')||'':'';}
  ...
  function labelBack(){var button=d.querySelector('[data-action="back"]');if(!button)return false;button.setAttribute('data-parent-route',parentRoute(location.pathname));if(sourceMode()){button.setAttribute('title','回到当前页面所属的上一级有效页面');button.setAttribute('aria-label','回上一层');}return true;}
  function markCurrentNav(){var path=normalizedPath(location.pathname);var routes=['/','/experience/','/capabilities/','/improvements/','/projects/','/trust/','/cooperation/','/knowledge/'];var current='/';for(var i=1;i<routes.length;i++)if(path.indexOf(routes[i])===0){current=routes[i];break;}d.querySelectorAll('.qily-global-nav a[href],nav.site-nav a[href]').forEach(function(link){var href=normalizedPath(link.getAttribute('href')||'');if(href===current)link.setAttribute('aria-current','page');else link.removeAttribute('aria-current');});}
  function boot(){markCurrentNav();if(!labelBack()){w.setTimeout(labelBack,160);w.setTimeout(labelBack,600);}}
  d.addEventListener('qily:language-change',labelBack);
