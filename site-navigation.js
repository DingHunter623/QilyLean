/* QilyLean navigation runtime v44｜2026-08-27
 * R6 search relevance + terminology exact-code + primary-navigation parity closure:
 ...
 * 5) primary navigation uses one sitewide typography/color contract on desktop and mobile; modules may not shrink it;
 * 6) site search navigation is preloaded before dock/core fallback, with R6 ranking remaining presentation-only;
 * 7) terminology live metadata uses one canonical strip with explicit light-surface contrast ownership;
 * 8) existing dock, resource collaboration, search authority and capability self-heal behavior remains intact.
 */
(function (d, w) {
  'use strict';
  if (w.__qilyStaticFirstNavigationV44) return;
  ...
  var CONSISTENCY_SRC = '/site-ui-consistency-v1.js?v=20260827-translation-dock-resource-v46';
  ...
  function clearLegacyDockPosition() { try { w.localStorage.removeItem('qilyDockPositionV2'); w.localStorage.removeItem('qilyDockTop'); } catch (error) {} }
  function snapDockHome() {
    clearLegacyDockPosition(); var dock = d.getElementById('floatDock'); if (!dock) return false;
    dock.style.setProperty('left', 'auto', 'important'); dock.style.setProperty('right', 'max(var(--qily-dock-edge, 12px), env(safe-area-inset-right))', 'important'); dock.style.setProperty('top', 'auto', 'important'); dock.style.setProperty('bottom', 'max(var(--qily-dock-edge, 12px), env(safe-area-inset-bottom))', 'important'); dock.dataset.qilyDockHome = 'bottom-right'; return true;
  }
  function ensureSearchAuthorityNavigation() {
    var path = currentPath(); var active = path === LEAN_AUTHORITY_PATH || path.indexOf(LEAN_AUTHORITY_PATH) === 0; var sourceMode = isChineseSourceMode(); var changed = false;
    d.querySelectorAll('.qily-global-nav,header nav.site-nav,header nav.nav,header nav[aria-label="网站导航"],header nav[aria-label="QilyLean核心导视"],header nav').forEach(function (nav) {
      ...
      link.setAttribute('data-qily-search-authority', 'lean-production'); if (sourceMode && link.getAttribute('aria-label') !== '精益生产专题') link.setAttribute('aria-label', '精益生产专题'); if (active) link.setAttribute('aria-current', 'page'); else if (link.getAttribute('aria-current') === 'page') link.removeAttribute('aria-current');
    }); return changed;
  }
  function normalizeResourceCollaborationLabel() {
    var changed = false; var sourceMode = isChineseSourceMode();
    d.querySelectorAll('header nav a[href="/links/"],header nav a[href="/links/index.html"],.qily-global-nav a[href="/links/"],.qily-global-nav a[href="/links/index.html"]').forEach(function (link) { if (sourceMode && (link.textContent || '').trim() !== '资源协同') { link.textContent = '资源协同'; changed = true; } if (sourceMode && link.getAttribute('aria-label') !== '资源协同') link.setAttribute('aria-label', '资源协同'); }); return changed;
  }
  function closeRuntimeGap() { ensureSearchAuthorityNavigation(); normalizeResourceCollaborationLabel(); snapDockHome(); w.requestAnimationFrame(function () { ensureSearchAuthorityNavigation(); normalizeResourceCollaborationLabel(); snapDockHome(); }); }
  function bindPermanentClosure() {
    if (w.__qilyResourceCollabDockHomeBoundV44) return; w.__qilyResourceCollabDockHomeBoundV44 = true;
    d.addEventListener('pointerup', function () { w.requestAnimationFrame(snapDockHome); }, false); d.addEventListener('pointercancel', function () { w.requestAnimationFrame(snapDockHome); }, false); w.addEventListener('resize', function () { w.requestAnimationFrame(snapDockHome); }, { passive: true }); w.addEventListener('pageshow', function () { w.requestAnimationFrame(closeRuntimeGap); }, { passive: true }); d.addEventListener('qily:shell-ready', closeRuntimeGap); d.addEventListener('qily:language-change', function () { w.requestAnimationFrame(closeRuntimeGap); });
    var root = d.documentElement || d.body;
    if (root && w.MutationObserver) { var queued = false; var observer = new MutationObserver(function () { if (queued) return; queued = true; w.requestAnimationFrame(function () { queued = false; ensureSearchAuthorityNavigation(); normalizeResourceCollaborationLabel(); var dock = d.getElementById('floatDock'); if (dock && dock.dataset.qilyDockHome !== 'dragging') snapDockHome(); }); }); observer.observe(root, { childList: true, subtree: true }); }
  }
  ...
  function needsLegacyRuntime() { var path = currentPath(); return path.indexOf('/cooperation/') === 0 || path.indexOf('/links/') === 0; }
  function installCapabilitySelfHeal() { if (currentPath() !== '/capabilities/') return; ensureStylesheet('qilyPureDdzCapabilityVisualV2', CAPABILITY_DDZ_HREF, 'link[href*="/pure-ddz-capability-visual-v2.css"]'); if (!d.querySelector('script[src*="/app-download-share-v1.js?v=20260824-capability-home-actions-v2"]')) loadScript('qilyAppDownloadShareRuntime', APP_SHARE_SRC); }
  function loadRuntime() {
    loadScript('qilySiteSearchRuntimeV2', SEARCH_RUNTIME_SRC); loadScript('qilyR6SearchTerminologyVisualV1', R6_SEARCH_VISUAL_SRC); loadScript('qilyPublicIntegrityHotfixV1', INTEGRITY_SRC); installCapabilitySelfHeal();
    loadScript('qilySiteNavigationCoreScript', CORE_SRC, function () { closeRuntimeGap(); installCapabilitySelfHeal(); if (needsLegacyRuntime()) loadScript('qilySiteNavigationLegacyScriptV32', LEGACY_SRC, closeRuntimeGap); if (!w.__qilyUiConsistencyV5 && !d.querySelector('script[src*="/site-ui-consistency-v1.js"]')) loadScript('qilyUiConsistencyRuntimeV33', CONSISTENCY_SRC, closeRuntimeGap); });
  }
