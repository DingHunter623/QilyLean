(() => {
  ...
  const JOKER_THEME=Object.freeze({16:{type:'small-joker',title:'小王',code:'C919',subtitle:'六大业务飞机模型',image:assetUrl('airplane-joker.png')},17:{type:'big-joker',title:'大王',code:'QILY',subtitle:'QilyLean 体系设计者',image:assetUrl('avatar-king.webp')}});
  ...
  function renderJoker(card){const theme=getTheme(card);return `<span class="qily-card qily-card--joker ${theme.type}"><span class="qily-card-corner"><b>${escapeHtml(theme.title)}</b><i>JOKER</i></span><span class="qily-joker-visual"><img src="${escapeHtml(theme.image)}" alt="${escapeHtml(theme.title)}" draggable="false"></span><span class="qily-card-main"><strong>${escapeHtml(theme.code)}</strong><em>${escapeHtml(theme.subtitle)}</em></span></span>`}
  ...
  function renderMiniCard(card){const theme=getTheme(card);if(card.rank>=16)return `<span class="mini-card qily-mini-joker ${theme.type}"><img src="${escapeHtml(theme.image)}" alt=""><b>${escapeHtml(theme.title)}</b></span>`;return `<span class="mini-card qily-mini-business${card.suit==='♥'||card.suit==='♦'?' red':''}"><b>${escapeHtml(ruleRankText(card))}${escapeHtml(card.suit)}</b><small>${escapeHtml(theme.code)}</small></span>`}
