/* QilyLean Pure DDZ site shell adapter v1.4.0｜2026-09-02
 * Pure DDZ is now a normal QilyLean page. This page-scoped adapter restores the canonical six-action Dock
 * because the legacy global Dock runtime intentionally excluded /tools/pure-ddz/ when it was immersive.
 */
(function(d,w){
  'use strict';
  if(w.__qilyDdzSiteShellV140)return;
  ...
  function ensureDock(){
    ...
    dock.replaceChildren(fragment);dock.dataset.qilyStableOrder=ORDER.join(',');dock.dataset.qilyDdzSiteDock='v1.4.0';
    return dock;
  }
  ...
  function toast(message){var node=d.getElementById('qilyDdzDockToastV140');if(!node){node=d.createElement('div');node.id='qilyDdzDockToastV140';node.setAttribute('role','status');node.style.cssText='position:fixed;left:50%;bottom:22px;z-index:2147483000;transform:translateX(-50%);padding:9px 14px;border-radius:999px;background:#073c47;color:#fff;font:800 14px/1.2 sans-serif;box-shadow:0 8px 24px rgba(0,0,0,.22);pointer-events:none;opacity:0;transition:opacity .16s';d.body.appendChild(node);}node.textContent=message;node.style.opacity='1';clearTimeout(toast.timer);toast.timer=setTimeout(function(){node.style.opacity='0';},2200);}
  function openSearch(){
    if(w.QilySiteSearch&&typeof w.QilySiteSearch.open==='function'){w.QilySiteSearch.open();return;}
    var existing=d.getElementById('qilyDdzSearchRuntimeV140');if(existing)return;
    var script=d.createElement('script');script.id='qilyDdzSearchRuntimeV140';script.src='/site-search.js?v=20260826-search-navigation-v2';
  ...
  function bindDock(){var dock=ensureDock();if(dock.dataset.qilyDdzBound==='true')return;dock.dataset.qilyDdzBound='true';dock.addEventListener('click',function(event){var button=event.target.closest&&event.target.closest('.qily-float-btn[data-action]');if(!button||!dock.contains(button))return;event.preventDefault();run(button.getAttribute('data-action'));});}
  function init(){bindDock();d.body&&d.body.setAttribute('data-parent-route','/');}
  if(d.readyState==='loading')d.addEventListener('DOMContentLoaded',init,{once:true});else init();
  w.addEventListener('pageshow',ensureDock,{passive:true});
})(document,window);
