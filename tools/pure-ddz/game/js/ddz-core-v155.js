...
(() => {
  ...
  const state = {phase:'idle',hands:[[],[],[]],bottom:[],landlord:null,current:0,lastPlay:null,tablePlays:[null,null,null],tablePasses:[false,false,false],trickNumber:0,passCount:0,selected:new Set(),winner:null,bids:[null,null,null],bidStart:0,bidTurns:0,highestBid:0,highestBidder:null,baseScore:1,multiplier:1,playCounts:[0,0,0],roundNumber:0,turnTimer:null,humanTurnTimer:null,humanTurnInterval:null,humanDeadline:0,musicTimer:null,audioCtx:null,musicMode:'normal',flowToken:0,profile:loadJson(PROFILE_KEY,DEFAULT_PROFILE),settings:loadJson(SETTINGS_KEY,DEFAULT_SETTINGS)};
  ...
  function renderTablePlays(){
    let board=$('ddz-played-table');
    if(!board){board=document.createElement('section');board.id='ddz-played-table';board.setAttribute('aria-label','三位玩家最近出牌');document.querySelector('.table-wrap').appendChild(board);}
    board.hidden=!['playing','ended'].includes(state.phase);
    if(board.hidden){board.innerHTML='';board._ddzMarkup='';return;}
    const html=[1,0,2].map(player=>{
      const play=state.tablePlays[player],active=Boolean(state.lastPlay&&state.lastPlay.player===player),passed=state.tablePasses[player];
      const badge=state.phase==='ended'?(state.winner===player?'已出完':'本局结束'):active?'当前出牌':passed?'不要':play?(!state.lastPlay&&state.current===player?'待领出':play.trickNumber===state.trickNumber?'已被压过':'上一轮'):'尚未出牌';
      const cards=play?.cards||[],role=player===state.landlord?'地主':'农民';
      const rows=cards.length>10?[cards.slice(0,Math.ceil(cards.length/2)),cards.slice(Math.ceil(cards.length/2))]:[cards];
      const faces=cards.length?rows.map(row=>`<div class="ddz-play-row">${row.map(card=>`<span class="ddz-play-card${isRed(card)?' is-red':''}${card.rank>=16?' is-joker':''}" data-card-id="${card.id}" aria-label="${cardLabel(card)}"><b>${rankText(card.rank)}</b><i aria-hidden="true">${card.rank>=16?'★':card.suit}</i></span>`).join('')}</div>`).join(''):'<span class="ddz-play-empty">等待出牌</span>';
      return `<section class="ddz-player-play${active?' is-current':''}" data-player="${player}" aria-label="${playerName(player)}最近出牌"><div class="ddz-play-heading"><strong data-count="${cards.length?' · '+cards.length+' 张':''}">${playerName(player)}<small> · ${role}</small></strong><span class="ddz-play-badge">${badge}</span></div><div class="ddz-play-description">${play?`${comboText(play.combo)} · ${cards.length} 张`:'最近出牌'}</div><div class="ddz-play-cards">${faces}</div></section>`;
    }).join('');
    if(board._ddzMarkup!==html){board.innerHTML=html;board._ddzMarkup=html;}
    fitTablePlays();
  }
  function fitTablePlays(){
    const board=$('ddz-played-table'),table=document.querySelector('.table-wrap'),center=document.querySelector('.center-area'),me=$('me-panel'),left=$('left-panel'),right=$('right-panel');
    if(!board||board.hidden||!table||!center||!me||!left||!right)return;
    // offset coordinates remain correct when the mobile table is rotated or scaled.
    const stacked=document.documentElement.classList.contains('ddz-mobile-portrait')&&!document.documentElement.classList.contains('ddz-ios-virtual-landscape');
    board.classList.toggle('is-stacked',stacked);
    document.documentElement.classList.toggle('ddz-short-table',!stacked&&table.clientHeight<640);
    const top=Math.max(center.offsetTop+center.offsetHeight,stacked?Math.max(left.offsetTop+left.offsetHeight,right.offsetTop+right.offsetHeight):0)+8,bottom=me.offsetTop-8;
    board.style.top=`${top}px`;board.style.height=`${Math.max(70,bottom-top)}px`;
    board.style.left=`${stacked?8:left.offsetLeft+left.offsetWidth+10}px`;
    board.style.right=`${stacked?8:Math.max(8,table.clientWidth-right.offsetLeft+10)}px`;
    board.classList.toggle('is-compact',bottom-top<155);
    board.classList.toggle('is-tight',bottom-top<108);
    board.querySelectorAll('.ddz-play-cards').forEach(hand=>{
      const rows=[...hand.querySelectorAll('.ddz-play-row')];if(!rows.length)return;
      const h=Math.max(24,Math.min(118,(hand.clientHeight-4*(rows.length-1))/rows.length));
      rows.forEach(row=>{
        const count=row.children.length,w=Math.min(80,h*.68),step=count>1?Math.min(w+5,(row.clientWidth-w)/(count-1)):w;
        row.style.setProperty('--played-w',`${w}px`);row.style.setProperty('--played-h',`${h}px`);
        row.style.setProperty('--played-overlap',`${Math.min(0,step-w)}px`);
        row.style.setProperty('--played-rank',`${Math.max(9,Math.min(30,h*.28,step*.78))}px`);
      });
    });
  }
  function renderLastPlay(){
    renderTablePlays();
    if(state.lastPlay){$('center-play').innerHTML=`<div class="play-owner">${describePlay(state.lastPlay)}</div>`;return;}
    const copy=state.phase==='bidding'?'正在叫地主，分数最高者获得三张底牌':state.phase==='playing'?`新一轮，${playerName(state.current)}领出`:'点击“开始游戏”，无需登录即可游玩';
  ...
  function startRound(){clearTimeout(state.turnTimer);clearHumanTurnClock();state.flowToken++;try{speechSynthesis.cancel();}catch(_error){}closeModal('welcome');closeModal('result');if(window.QilyLeanExpertAI)window.QilyLeanExpertAI.memory.reset();state.phase='bidding';state.hands=[[],[],[]];state.bottom=[];state.landlord=null;state.lastPlay=null;state.tablePlays=[null,null,null];state.tablePasses=[false,false,false];state.trickNumber=0;state.passCount=0;state.selected.clear();state.winner=null;state.bids=[null,null,null];state.bidTurns=0;state.highestBid=0;state.highestBidder=null;state.baseScore=1;state.multiplier=1;state.playCounts=[0,0,0];const deck=shuffle(createDeck());state.hands=[deck.slice(0,17),deck.slice(17,34),deck.slice(34,51)];state.bottom=deck.slice(51);state.hands.forEach(sortHand);state.bidStart=Math.floor(Math.random()*3);state.current=state.bidStart;state.roundNumber=state.profile.games+1;$('hint-message').textContent='';render();try{startMusic();}catch(error){console.warn('Pure DDZ music unavailable:',error);}afterNarration('开始叫地主',scheduleTurn,500);}
  ...
  window.addEventListener('resize',()=>requestAnimationFrame(fitTablePlays),{passive:true});
  if(window.ResizeObserver){const observer=new ResizeObserver(fitTablePlays);['.table-wrap','.center-area','#me-panel'].forEach(selector=>{const el=document.querySelector(selector);if(el)observer.observe(el);});}
