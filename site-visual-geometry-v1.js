/* QilyLean visual geometry closure v1｜2026-08-19
...
(function (d, w) {
  'use strict';
  if (w.__qilyVisualGeometryV1) return;
  w.__qilyVisualGeometryV1 = true;
  ...
  function tightenSceneSvg(svg) {
    if (!svg || svg.dataset.qilyGeometryTightened === '1') return;
    ...
    if (usage >= 0.925) {
      svg.dataset.qilyGeometryTightened = '1';
      return;
    }

    var pad = Math.max(24, Math.min(42, viewBox.width * 0.028));
    ...
    if (nextWidth < viewBox.width * 0.72 || nextWidth >= viewBox.width * 0.96) {
      svg.dataset.qilyGeometryTightened = '1';
      return;
    }

    if (background) {
      var inset = Math.max(10, Math.min(22, nextWidth * 0.015));
