/* QilyLean Sitewide Content Contrast Guard V4｜2026-08-26
 ...
 * V4: any gradient/image-backed surface is component-owned. The generic scanner must not
 * infer a white background from transparent background-color and repaint authored light text dark.
 */
(function(d,w){
  'use strict';
  ...
  function componentOwnsContrast(el){
    return !!(el&&el.closest&&(el.closest(COMPONENT_OWNED_DARK)||hasVisualBackground(el)));
  }
  function fix(el){
  ...
  if(w.MutationObserver)new MutationObserver(function(records){for(var i=0;i<records.length;i+=1){var target=records[i].target&&records[i].target.nodeType===1?records[i].target:records[i].target&&records[i].target.parentElement;if(target){schedule(target);break}}}).observe(d.documentElement,{subtree:true,childList:true,characterData:true,attributes:true,attributeFilter:['class','style']});
})(document,window);