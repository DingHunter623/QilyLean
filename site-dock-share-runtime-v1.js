...
(function(d,w){
  ...
  var LABELS={home:['回首页'],top:['回顶部'],back:['回上一层级'],previous:['回上一网页'],search:['本站搜索'],current:['分享当前页'],contact:['联系我们']};
  ...
  function ensureStyles(){
    ...
    var style=d.createElement('style');style.id='qilyDockUnifiedV56Style';
    style.textContent=[
      ...
      'html:root:root body #floatDock.qily-page-action-nav{position:relative!important;inset:auto!important;z-index:20!important;display:grid!important;grid-template-columns:repeat(7,minmax(0,1fr))!important;align-items:stretch!important;width:min(var(--qily-container,1240px),calc(100% - 36px))!important;max-width:var(--qily-container,1240px)!important;height:auto!important;gap:var(--qily-dock-gap)!important;margin:var(--qily-dock-section-gap) auto!important;padding:0!important;border:0!important;border-radius:0!important;background:transparent!important;box-shadow:none!important;overflow:visible!important;transform:none!important}',
      'html:root:root body #floatDock.qily-page-action-nav .qily-float-btn.qily-float-btn{display:flex!important;align-items:center!important;justify-content:center!important;box-sizing:border-box!important;flex:1 1 auto!important;width:100%!important;height:auto!important;min-width:0!important;min-height:52px!important;max-width:none!important;max-height:none!important;aspect-ratio:auto!important;margin:0!important;padding:11px 12px!important;border:1px solid var(--qily-dock-line)!important;border-radius:8px!important;color:var(--qily-dock-text)!important;-webkit-text-fill-color:var(--qily-dock-text)!important;background:var(--qily-dock-bg)!important;background-image:none!important;box-shadow:none!important;font:850 15px/1.3 "Microsoft YaHei","PingFang SC",Arial,sans-serif!important;text-align:center!important;white-space:nowrap!important;cursor:pointer!important;appearance:none!important;-webkit-appearance:none!important;touch-action:manipulation!important}',
      ...
      '@media(max-width:820px){:root{--qily-dock-size:52px;--qily-dock-gap:10px}html:root:root body #floatDock.qily-page-action-nav{display:flex!important;grid-template-columns:none!important;flex-wrap:nowrap!important;width:calc(100% - 24px)!important;max-width:none!important;gap:var(--qily-dock-gap)!important;padding:0 0 6px!important;overflow-x:auto!important;overflow-y:hidden!important;overscroll-behavior-inline:contain!important;scroll-snap-type:x proximity!important;scrollbar-width:thin!important;-webkit-overflow-scrolling:touch!important}html:root:root body #floatDock.qily-page-action-nav .qily-float-btn.qily-float-btn{flex:0 0 132px!important;width:132px!important;min-width:132px!important;max-width:156px!important;height:auto!important;min-height:50px!important;max-height:none!important;aspect-ratio:auto!important;padding:10px 12px!important;scroll-snap-align:start!important;font-size:14.5px!important}}',
      '@media(max-width:390px){:root{--qily-dock-size:50px;--qily-dock-gap:9px;--qily-dock-section-gap:20px}html:root:root body #floatDock.qily-page-action-nav{width:calc(100% - 20px)!important}html:root:root body #floatDock.qily-page-action-nav .qily-float-btn.qily-float-btn{flex-basis:132px!important;width:132px!important;min-width:132px!important;min-height:48px!important;font-size:14px!important}}',
      '@media print{html body #floatDock{display:none!important}}'
    ].join('');
  ...
  function setOwnedLabel(button,action){
    ...
    button.setAttribute('aria-label',lines.join(''));button.setAttribute('title',lines.join(''));button.setAttribute('data-qily-label-owner','dock-v5.6-flow');
  }
  function placeDockInFlow(dock){
    var footer=d.querySelector('footer'),main=d.querySelector('main');
    if(footer&&footer.parentNode){if(dock.parentNode!==footer.parentNode||dock.nextElementSibling!==footer)footer.parentNode.insertBefore(dock,footer);return;}
    if(main&&main.parentNode){if(main.nextSibling)main.parentNode.insertBefore(dock,main.nextSibling);else main.parentNode.appendChild(dock);}
  }
  ...
  function applyInlineGeometry(dock){
    ...
    ['top','right','bottom','left','inset'].forEach(function(name){setImportant(dock,name,'auto');});
    setImportant(dock,'position','relative');setImportant(dock,'z-index','20');setImportant(dock,'height','auto');setImportant(dock,'min-height','0');setImportant(dock,'padding',mobile?'0 0 6px':'0');setImportant(dock,'border','0');setImportant(dock,'border-radius','0');setImportant(dock,'background','transparent');setImportant(dock,'box-shadow','none');setImportant(dock,'transform','none');
    setImportant(dock,'margin',mobile&&w.innerWidth<=390?'20px auto':'24px auto');
    if(mobile){
      setImportant(dock,'display','flex');setImportant(dock,'flex-wrap','nowrap');setImportant(dock,'width',w.innerWidth<=390?'calc(100% - 20px)':'calc(100% - 24px)');setImportant(dock,'max-width','none');setImportant(dock,'gap',w.innerWidth<=390?'9px':'10px');setImportant(dock,'overflow-x','auto');setImportant(dock,'overflow-y','hidden');setImportant(dock,'scroll-snap-type','x proximity');setImportant(dock,'overscroll-behavior-inline','contain');
    }else{
      setImportant(dock,'display','grid');setImportant(dock,'grid-template-columns','repeat(7,minmax(0,1fr))');setImportant(dock,'width','min(var(--qily-container,1240px),calc(100% - 36px))');setImportant(dock,'max-width','var(--qily-container,1240px)');setImportant(dock,'gap','12px');setImportant(dock,'overflow','visible');
    }
    dock.querySelectorAll('.qily-float-btn').forEach(function(button){
      setImportant(button,'display','flex');setImportant(button,'align-items','center');setImportant(button,'justify-content','center');setImportant(button,'box-sizing','border-box');setImportant(button,'height','auto');setImportant(button,'max-height','none');setImportant(button,'aspect-ratio','auto');setImportant(button,'border-radius','8px');setImportant(button,'margin','0');setImportant(button,'padding',mobile?'10px 12px':'11px 12px');
      if(mobile){setImportant(button,'flex','0 0 132px');setImportant(button,'width','132px');setImportant(button,'min-width','132px');setImportant(button,'max-width','156px');setImportant(button,'min-height',w.innerWidth<=390?'48px':'50px');setImportant(button,'scroll-snap-align','start');}
      else{setImportant(button,'flex','1 1 auto');setImportant(button,'width','100%');setImportant(button,'min-width','0');setImportant(button,'max-width','none');setImportant(button,'min-height','52px');}
    });
  }
  ...
  function normalizeDock(){
    ...
    var dock=d.getElementById('floatDock')||createStandaloneDock();dock.className='qily-float-dock qily-page-action-nav';dock.hidden=false;dock.removeAttribute('aria-hidden');dock.setAttribute('aria-label','页面快捷导航');dock.setAttribute('data-qily-dock-layout','flow-navigation');
    ...
    dock.dataset.qilyStableOrder=ORDER.join(',');dock.dataset.qilyUnifiedPublicModule='v5.6-flow-navigation';dock.dataset.qilyDockLayout='flow-navigation';placeDockInFlow(dock);applyInlineGeometry(dock);return true;
  }
