/* QilyLean Floating Dock Unified Runtime V3｜2026-08-28
 * One public component, one visual and behavior contract sitewide.
 * Canonical actions: 首页 / ↑顶部 / ↗上一层 / 本站搜索 / 分享当前页 / 联系我们.
 * V3 can normalize an existing shell Dock OR create a standalone functional Dock for special pages
 * (OPL, document previews, utility pages) that intentionally do not load the full site-navigation shell.
 */
(function(d,w){
  'use strict';
  ...
  function ensureStyles(){
    if(d.getElementById('qilyDockUnifiedV3Style'))return;
    var style=d.createElement('style');
    style.id='qilyDockUnifiedV3Style';
    style.textContent=[
      ':root{--qily-dock-v3-bg:#0f4b5a;--qily-dock-v3-hover:#12606f;--qily-dock-v3-line:#caa15f;--qily-dock-v3-text:#ffe39b}',
      ...
      'html body #floatDock .qily-float-btn{display:flex!important;flex:0 0 64px!important;width:64px!important;height:64px!important;min-width:64px!important;min-height:64px!important;max-width:64px!important;max-height:64px!important;align-items:center!important;justify-content:center!important;gap:1px!important;margin:0!important;padding:5px!important;border:1.5px solid var(--qily-dock-v3-line)!important;border-radius:999px!important;color:var(--qily-dock-v3-text)!important;-webkit-text-fill-color:var(--qily-dock-v3-text)!important;background:var(--qily-dock-v3-bg)!important;background-color:var(--qily-dock-v3-bg)!important;background-image:none!important;box-shadow:0 8px 20px rgba(7,60,71,.22)!important;opacity:1!important;filter:none!important;mix-blend-mode:normal!important;font:850 14px/1.05 "Microsoft YaHei","PingFang SC",Arial,sans-serif!important;letter-spacing:0!important;text-align:center!important;text-decoration:none!important;cursor:pointer!important;appearance:none!important;-webkit-appearance:none!important}',
      'html body #floatDock .qily-float-btn *,html body #floatDock .qily-float-btn:is(:link,:visited,:active){color:var(--qily-dock-v3-text)!important;-webkit-text-fill-color:var(--qily-dock-v3-text)!important}',
      'html body #floatDock .qily-float-btn:is(:hover,:focus-visible){background:var(--qily-dock-v3-hover)!important;background-color:var(--qily-dock-v3-hover)!important;background-image:none!important;border-color:var(--qily-dock-v3-text)!important;outline:3px solid rgba(202,161,95,.22)!important;outline-offset:2px!important}',
      'html body #floatDock [data-action="top"],html body #floatDock [data-action="back"]{flex-direction:column!important;gap:0!important}',
      'html body #floatDock .qily-dock-semantic-icon{display:grid!important;place-items:center!important;width:18px!important;height:18px!important;flex:0 0 18px!important;margin:0!important;color:currentColor!important;-webkit-text-fill-color:currentColor!important;font:950 19px/1 Arial,sans-serif!important}',
  ...
  function createStandaloneDock(){
    ...
    dock.id='floatDock';dock.className='qily-float-dock';dock.dataset.qilyStandaloneDock='v3';dock.setAttribute('aria-label','快捷服务');
    dock.innerHTML=[
  ...
  function normalizeSemanticButton(button,action,label){
    ...
    icon.replaceChildren();icon.textContent=action==='top'?'↑':'↗';
  ...
  function normalizeDock(){
    ...
    dock.dataset.qilyStableOrder=ORDER.join(',');dock.dataset.qilyUnifiedPublicModule='v3';
  ...
  function toast(message){
    var node=d.getElementById('qilyDockStandaloneToastV3');if(!node){node=d.createElement('div');node.id='qilyDockStandaloneToastV3';node.setAttribute('role','status');node.style.cssText='position:fixed;left:50%;bottom:22px;z-index:2147483000;transform:translateX(-50%);padding:9px 14px;border-radius:999px;background:#073c47;color:#fff;font:700 14px/1.2 sans-serif;box-shadow:0 8px 24px rgba(0,0,0,.22);pointer-events:none;opacity:0;transition:opacity .16s';(d.body||d.documentElement).appendChild(node);}node.textContent=message;node.style.opacity='1';clearTimeout(toast.timer);toast.timer=setTimeout(function(){node.style.opacity='0';},2200);
  }
  function openSearch(){
    if(w.QilySiteSearch&&typeof w.QilySiteSearch.open==='function'){w.QilySiteSearch.open();return;}
    var existing=d.getElementById('qilySiteSearchStandaloneV3');if(existing){existing.addEventListener('load',function(){if(w.QilySiteSearch)w.QilySiteSearch.open();},{once:true});return;}
    var script=d.createElement('script');script.id='qilySiteSearchStandaloneV3';script.src='/site-search.js?v=20260729-ranked-search-v1';script.addEventListener('load',function(){if(w.QilySiteSearch)w.QilySiteSearch.open();else toast('本站搜索加载失败');},{once:true});script.addEventListener('error',function(){toast('本站搜索加载失败');},{once:true});(d.body||d.documentElement).appendChild(script);
  }
  ...
  function installStandaloneFallback(dock){
    if(dock.dataset.qilyFallbackBound==='v3')return;
    dock.dataset.qilyFallbackBound='v3';
    dock.addEventListener('click',function(event){
      if(w.__qilyLeanSiteNavigationPublicV8&&dock.dataset.qilyStandaloneDock!=='v3')return;
  ...
  function boot(){normalizeDock();d.addEventListener('qily:shell-ready',normalizeDock);d.addEventListener('qily:softnavigate',normalizeDock);d.addEventListener('qily:language-change',normalizeDock);w.addEventListener('pageshow',normalizeDock,{passive:true});}
