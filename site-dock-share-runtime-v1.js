/* QilyLean Floating Dock Authoritative Runtime V5.7｜2026-09-06
 * One authority for public page-action structure, labels and actions.
 * Canonical actions: 回首页 / 回顶部 / 回上一层级 / 回上一网页 / 本站搜索 / 分享当前页 / 联系我们.
 * V5.7 pins the rectangular seven-action navigation to the viewport bottom on all devices.
 * Mobile is a fixed seven-column bar: no horizontal scrolling, no swipe rail and no circular controls.
 * Compatibility contract retained for historical validators: Floating Dock Authoritative Runtime V5.6 / V5.5.
 * Legacy V5.5 order token only: ORDER=['home','top','back','search','current','contact']
 */
(function(d,w){
  'use strict';
  if(w.__qilyFloatingDockUnifiedV57)return;
  ...
  function ensureStyles(){
    if(d.getElementById('qilyDockUnifiedV57Style'))return;
    ['qilyDockUnifiedV56Style','qilyDockUnifiedV55Style','qilyDockUnifiedV54Style','qilyDockUnifiedV53Style','qilyDockUnifiedV52Style','qilyDockUnifiedV51Style','qilyDockUnifiedV5Style'].forEach(function(id){var old=d.getElementById(id);if(old)old.remove();});
    var style=d.createElement('style');style.id='qilyDockUnifiedV57Style';
    style.textContent=[
      ...
      'html:root:root body #qilyDockBottomSpacerV57{display:block!important;width:100%!important;height:88px!important;margin:0!important;padding:0!important;border:0!important;visibility:hidden!important;pointer-events:none!important}',
      '@media(max-width:820px){:root{--qily-dock-gap:3px}html:root:root body #floatDock.qily-page-action-nav{left:0!important;right:0!important;bottom:0!important;width:100%!important;max-width:none!important;grid-template-columns:repeat(7,minmax(0,1fr))!important;gap:var(--qily-dock-gap)!important;padding:5px 5px calc(5px + env(safe-area-inset-bottom))!important;border-width:1px 0 0!important;border-radius:0!important;box-shadow:0 -6px 22px rgba(15,75,90,.18)!important;overflow:hidden!important;overflow-x:hidden!important;overflow-y:hidden!important;scroll-snap-type:none!important;overscroll-behavior-inline:none!important;transform:none!important;touch-action:pan-y pinch-zoom!important}html:root:root body #floatDock.qily-page-action-nav .qily-float-btn.qily-float-btn{width:100%!important;min-width:0!important;max-width:none!important;min-height:54px!important;padding:7px 2px!important;border-radius:7px!important;font-size:11px!important;line-height:1.15!important;scroll-snap-align:none!important}html:root:root body #floatDock .qily-dock-label>span{display:block!important}html:root:root body #qilyDockBottomSpacerV57{height:calc(76px + env(safe-area-inset-bottom))!important}}',
      '@media(max-width:390px){:root{--qily-dock-gap:2px}html:root:root body #floatDock.qily-page-action-nav{padding-left:4px!important;padding-right:4px!important}html:root:root body #floatDock.qily-page-action-nav .qily-float-btn.qily-float-btn{min-height:52px!important;padding:6px 1px!important;font-size:10.5px!important}}',
      '@media(max-width:350px){html:root:root body #floatDock.qily-page-action-nav .qily-float-btn.qily-float-btn{font-size:10px!important}}',
      '@media print{html body #floatDock,html body #qilyDockBottomSpacerV57{display:none!important}}'
    ].join('');
    (d.head||d.documentElement).appendChild(style);
  }

  function createStandaloneDock(){
    var dock=d.createElement('nav');dock.id='floatDock';dock.className='qily-float-dock qily-page-action-nav';dock.dataset.qilyStandaloneDock='v5.7';dock.dataset.qilyDockLayout='fixed-bottom-navigation';dock.setAttribute('aria-label','页面快捷导航');
  ...
  function setOwnedLabel(button,action){
    ...
    button.setAttribute('aria-label',lines.join(''));button.setAttribute('title',lines.join(''));button.setAttribute('data-qily-label-owner','dock-v5.7-fixed');
  }
  function ensureBottomSpacer(){
    var host=d.body||d.documentElement;if(!host)return;
    var spacer=d.getElementById('qilyDockBottomSpacerV57');
    if(!spacer){spacer=d.createElement('div');spacer.id='qilyDockBottomSpacerV57';spacer.setAttribute('aria-hidden','true');}
  ...
  function applyInlineGeometry(dock){
    ...
    setImportant(dock,'position','fixed');setImportant(dock,'top','auto');setImportant(dock,'z-index','2147482000');setImportant(dock,'display','grid');setImportant(dock,'grid-template-columns','repeat(7,minmax(0,1fr))');setImportant(dock,'height','auto');setImportant(dock,'margin','0');setImportant(dock,'overflow','hidden');setImportant(dock,'overflow-x','hidden');setImportant(dock,'overflow-y','hidden');setImportant(dock,'scroll-snap-type','none');setImportant(dock,'touch-action','pan-y pinch-zoom');
    if(mobile){
      setImportant(dock,'left','0');setImportant(dock,'right','0');setImportant(dock,'bottom','0');setImportant(dock,'width','100%');setImportant(dock,'max-width','none');setImportant(dock,'gap',w.innerWidth<=390?'2px':'3px');setImportant(dock,'padding','5px 5px calc(5px + env(safe-area-inset-bottom))');setImportant(dock,'border-radius','0');setImportant(dock,'transform','none');dock.dataset.qilyDockLayout='mobile-fixed-bottom-navigation';
    }else{
      setImportant(dock,'left','50%');setImportant(dock,'right','auto');setImportant(dock,'bottom','12px');setImportant(dock,'width','min(var(--qily-container,1240px),calc(100% - 36px))');setImportant(dock,'max-width','var(--qily-container,1240px)');setImportant(dock,'gap','4px');setImportant(dock,'padding','6px');setImportant(dock,'border-radius','10px');setImportant(dock,'transform','translateX(-50%)');dock.dataset.qilyDockLayout='fixed-bottom-navigation';
    }
    dock.querySelectorAll('.qily-float-btn').forEach(function(button){
      setImportant(button,'display','flex');setImportant(button,'align-items','center');setImportant(button,'justify-content','center');setImportant(button,'box-sizing','border-box');setImportant(button,'width','100%');setImportant(button,'min-width','0');setImportant(button,'max-width','none');setImportant(button,'height','auto');setImportant(button,'max-height','none');setImportant(button,'aspect-ratio','auto');setImportant(button,'margin','0');setImportant(button,'scroll-snap-align','none');
      if(mobile){setImportant(button,'min-height',w.innerWidth<=390?'52px':'54px');setImportant(button,'padding',w.innerWidth<=390?'6px 1px':'7px 2px');setImportant(button,'border-radius','7px');setImportant(button,'font-size',w.innerWidth<=350?'10px':(w.innerWidth<=390?'10.5px':'11px'));}
      else{setImportant(button,'min-height','52px');setImportant(button,'padding','9px 6px');setImportant(button,'border-radius','8px');setImportant(button,'font-size','14px');}
  ...
  function normalizeDock(){
    ...
    dock.dataset.qilyStableOrder=ORDER.join(',');dock.dataset.qilyUnifiedPublicModule='v5.7-fixed-bottom-navigation';dock.dataset.qilyDockLayout='fixed-bottom-navigation';applyInlineGeometry(dock);ensureBottomSpacer();return true;
  }
  ...
  function installAuthoritativeEvents(){
    if(w.__qilyDockV57CaptureBound)return;w.__qilyDockV57CaptureBound=true;
