(function(){
 ...
 function addCard(){
   ...
   fetch('/qilylean/daily/index.json?latest='+Date.now(),{cache:'no-store'})
     ...
     .then(function(items){var latest=Array.isArray(items)&&items[0];if(latest&&/^\d{4}-\d{2}-\d{2}$/.test(latest.date||'')&&latest.date>=release.date)render(card,latest);})
     .catch(function(){})
